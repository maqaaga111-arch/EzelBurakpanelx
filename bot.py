...
VIP_USERNAME = os.environ.get('VIP_USERNAME', 'BurakEzel')
VIP_PASSWORD = os.environ.get('VIP_PASSWORD', 'BurakEzel0')
...
# (Dış host whitelist KALDIRILDI) - artık tüm hostlara proxy yapılabilir.
...
logger = logging.getLogger('b_fixed2_no_host_check')
...
def set_security_headers(resp):
    ...
    resp.headers['Content-Security-Policy'] = (
        ...
        "connect-src 'self' http://api.nabi.org.totalh.net/ http://localhost http://127.0.0.1:5000;"
    )
    return resp
...
# ---------- util: outbound (HOST CHECK KALDIRILDI) ----------
def fetch_api_get(url, params=None):
    """
    Fetch an external API using the configured session.
    - DOES NOT enforce allowed hostnames (kullanıcı isteği ile host kontrolü kaldırıldı).
    ...
    try:
        ...
        # NOT: host whitelist removed — sadece logluyoruz.
        logger.info('Proxying to %s (host=%s) params=%s', url, host, params)
...
API_PARAM_MAP = {
    ...
    'baba': ['tc'],
    'anne': ['tc'],
    # <-- changed here: only ad ve soyad zorunlu olacak
    'ad_soyad': ['ad','soyad'],
